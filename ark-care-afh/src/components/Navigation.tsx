...
export function Navigation() {
  ...
  return (
    <>
      ...
      {/* Pill-shaped Navbar with Overlay Effect */}
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 md:pt-8 pointer-events-none px-4 sm:px-6">
        <div className="pointer-events-auto w-full max-w-5xl bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-400 rounded-full shadow-2xl px-6 md:px-8 py-3 md:py-4">
            ...
            <div className="hidden lg:flex items-center gap-1 md:gap-2">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="px-3 md:px-4 py-2 text-xs md:text-sm font-medium text-white/90 hover:text-white transition-all duration-200 rounded-lg hover:bg-white/10"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="lg:hidden border-t border-white/20 mt-3 pt-3 space-y-2">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block px-4 py-2.5 text-sm font-medium text-white/90 hover:text-white hover:bg-white/10 rounded-lg transition-colors duration-200"
