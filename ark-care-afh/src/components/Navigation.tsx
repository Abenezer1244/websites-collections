...
export function Navigation() {
  ...
  return (
    <>
      ...
      <div className="fixed top-4 left-4 sm:left-6 z-50 pointer-events-auto">
        <Link
          href="/"
      ...
      {/* Theme Toggle - Fixed Position Top-Center */}
      <div className="fixed top-5 left-1/2 transform -translate-x-1/2 z-50 pointer-events-auto hidden md:block">
        <ModeToggle />
      </div>

      {/* Call Button - Fixed Position Top-Right */}
      <a
        href="tel:+12064553644"
        className="fixed top-5 right-4 sm:right-6 z-50 pointer-events-auto hidden md:inline-flex items-center justify-center w-10 h-10 md:w-12 md:h-12 bg-white/20 hover:bg-white/30 text-white rounded-full transition-all duration-200 border border-white/30"
        aria-label="Call now"
        title="Call Now"
      >
      ...
      {/* Floating Glassmorphic Navbar - Only Navigation Links */}
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 md:pt-8 pointer-events-none px-4 sm:px-6">
        <div className="pointer-events-auto w-full max-w-5xl bg-white/10 backdrop-blur-md border border-white/20 rounded-full shadow-2xl px-4 md:px-8 py-3 md:py-4 animate-float">
